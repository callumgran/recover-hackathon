...
def main():
    ...
    label_counts = compute_label_counts(train_dataset, batch_size=512)
    pos_weight = 1.0 / (label_counts + 1e-3)
    pos_weight = np.clip(pos_weight, 1.0, 100.0)
    pos_weight_tensor = torch.tensor(pos_weight, dtype=torch.float32, device=device)
    print("[weights] pos_weight stats -> min:", float(pos_weight_tensor.min()),
          "max:", float(pos_weight_tensor.max()),
          "mean:", float(pos_weight_tensor.mean()))
